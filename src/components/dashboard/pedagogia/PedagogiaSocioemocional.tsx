...
export function PedagogiaSocioemocional({
  ...
  useEffect(() => {
    if (!selectedBeneficiarioId || !projetoId || !mesReferencia) return;

    const carregarAvaliacao = async () => {
      setLoading(true);
      const supabase = createClient();

      // 1. Registro do mês específico
      ...
      // 2. Histórico de todos os meses deste aluno
      const { data: hist } = await supabase
        .from('acompanhamento_socioemocional')
        .select('id, mes_referencia, data_registro, responsavel_preenchimento, eixo4_evolucao_observada')
        .eq('beneficiario_id', selectedBeneficiarioId)
        .eq('projeto_id', projetoId)
        .order('mes_referencia', { ascending: false });

      setHistoricoAluno(hist || []);
      setLoading(false);
    };

    carregarAvaliacao();
  }, [selectedBeneficiarioId, projetoId, mesReferencia]);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Button
              ...
              onClick={() => setShowDevolutivaModal(true)}
        ...
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-elevated)] shadow-[var(--shadow-card)] p-4 sm:p-5 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-[var(--text-muted)]">
                Ações lúdicas ou de escuta individualizada planejadas para o próximo mês para apoiar essa criança.
              </p>
              <textarea
                rows={3}
      ...
      <PapelTimbradoModal
        ...
        onClose={() => setShowDevolutivaModal(false)}
        tituloDocumento="Devolutiva Socioemocional & Pedagógica à Família"
        subtituloDocumento={`Aluno: ${beneficiarioAtual?.nome_completo || ''} • Mês: ${mesReferencia}`}
      >
        <div className="space-y-5 text-xs text-slate-800 leading-relaxed">
          ...
          <div className="border border-slate-300 rounded-xl p-3.5 bg-slate-50 space-y-1.5 timbrado-avoid-break">
            ...
            <p><strong>Responsável pelo Acompanhamento:</strong> {responsavel || 'Equipe Pedagógica e Socioemocional'}</p>
          </div>
          ...
          <div className="border border-slate-300 rounded-xl p-4 space-y-3 timbrado-avoid-break">
            ...
            <div className="grid grid-cols-2 gap-3 text-[11px]">
              <div>
                ...
                <p className="text-slate-700">{formData.eixo1_expressao_opinioes} • {formData.eixo1_autoimagem}</p>
              </div>
              <div>
                <p className="font-semibold text-slate-900">Socialização & Coletivo:</p>
                <p className="text-slate-700">{formData.eixo2_resolucao_conflitos} • {formData.eixo2_trabalho_equipe}</p>
              </div>
              <div>
                <p className="font-semibold text-slate-900">Respeito à Rotina & Ritmo:</p>
                <p className="text-slate-700">{formData.eixo3_respeito_tempo_ritmo}</p>
              </div>
              <div>
                <p className="font-semibold text-slate-900">Escuta & Empatia:</p>
                <p className="text-slate-700">{formData.eixo3_escuta_ativa} • {formData.eixo3_apoio_mutuo}</p>
              </div>
            </div>
          </div>
          ...
          {formData.eixo4_evolucao_observada && (
            <div className="border border-slate-300 rounded-xl p-3.5 space-y-1 timbrado-avoid-break">
              ...
              <p className="text-[11px] text-slate-700">{formData.eixo4_evolucao_observada}</p>
            </div>
          )}
